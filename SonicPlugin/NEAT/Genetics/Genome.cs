        ...
        public void ConnectLayers(double minWeight, double maxWeight, double probability)
        {
            ...
            if (hiddenNodes.Length > 0)
            {
                for (int i = 0; i < inputs.Length; i++)
                {
                    ...
                    {
                        if (Connections.Where(cn => ((cn.Value.InputNode == inputs[i].NodeNumber) && (cn.Value.OutputNode == hiddenNodes[j].NodeNumber))).Count() == 0)
                        {
                ...
                for (int i = 0; i < hiddenNodes.Length; i++)
                {
                    ...
                    {
                        if (Connections.Where(cn => ((cn.Value.InputNode == hiddenNodes[i].NodeNumber) && (cn.Value.OutputNode == outputs[j].NodeNumber))).Count() == 0)
                        {
            ...
            else
            {
                for (int i = 0; i < inputs.Length; i++)
                {
                    for (int j = 0; j < outputs.Length; j++)
                    {
                        if (Connections.Where(cn => ((cn.Value.InputNode == inputs[i].NodeNumber) && (cn.Value.OutputNode == outputs[j].NodeNumber))).Count() == 0)
                        {
                            if (Random.NextBool(probability))
